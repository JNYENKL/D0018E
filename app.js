...
app.get('/p', (req, res)=> {
	  ...
	  db.query('SELECT * FROM comment WHERE order_asset_id=?', req.query.product, function(err, row, fields) {
		if (err) {
			session.message = "Technical issues, check back later.";
			res.render('index', {message: session.message});
			//res.status(500).json({"status_code": 500,"status_message": "internal server error"+ err});
	  ...
	  db.query('SELECT * FROM subject', function(err, row, fields) {
		if (err) {
			...
			// Rendera index.pug med objekten i listan
			res.render('productPage', {items: items, cat: cats, af: session.admin, login: session.loggedIn, message: "", comments: commentList});
		}
	  });
});
...
app.get('/cart', function(req, res) {
	...
	var query = "SELECT title, asset_id, asset_amount, price from shopping_basket sb join shopping_basket_asset sba using (shopping_basket_id) join asset a using (asset_id) where user_id=?";
	  	...
	  	if (err) {
			...
			console.log(rows);
			  ...
			  for (var i = 0; i < rows.length; i++) {
				...
				totalPrice= totalPrice + (parseInt(rows[i].price)*parseInt(rows[i].asset_amount));
	...
	db.query('SELECT * FROM subject', function(err, row, fields) {
		if (err) {
			...
			// Rendera index.pug med objekten i listan
			res.render('cart', {shoppingCart: itemList, login: session.loggedIn, cat: cats, af: session.admin, login: session.loggedIn, message: "", total: totalPrice});
		}
	  });
...
app.get('/removeFromCart', function(req, res) {
	...
	var query = 'DELETE from shopping_basket_asset WHERE user_id='+user+' AND asset_id='+productID;
	...
	if(ses.admin){
		...
		db.query('SELECT * FROM subject', function(err, row, fields) {
			if (err) {
				...
				// Rendera index.pug med objekten i listan
				res.render('adminPage', {orders: orderList, login: session.loggedIn, cat: cats, af: session.admin, login: session.loggedIn, message: ""});
			}
		  });
	} else {
		res.redirect('/');
